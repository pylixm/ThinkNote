...
zookeeper 目前最新版本为 3.5.6，需要之上JDK1.8以上，否则回报如下错误：
...
ZK 集群模式下，需要过半的机器是正常运行状态便可正常运行，所以建议集群机器数为奇数。例如5台设备的机器，允许你坏两台设备。可参考[这里](http://zookeeper.apache.org/doc/r3.5.6/zookeeperAdmin.html#sc_zkMulitServerSetup)。

如下我们搭建ZK最小集群，三台设备的集群：
...
- Step4：载以同样的发放部署其他两台设备

- Step5：标识Server ID

在dataDir目录冲创建集群机器的唯一标识文件`myid`的文件（每台服务器一个）。该ID在集群中必须唯一，且值介于1到255之间。启用SSL的集群，值介于1到254之间。
