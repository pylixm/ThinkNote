...
随着Docker的普及成熟，已经逐渐成为部署项目的首选，今天来和大家分享如何使用docker来部署django技术栈的项目。我们这里说的Django技术栈为：python3.6、Django2.2、redis、mysql、celery、gunicorn和nginx。在实际的生产项目中，这些组件分布在集群的不同机器，甚至Nginx、redis和Mysql可能会有单独的团队或部门负责，涉及的部署架构和容器的编排会更为复杂，本文暂不去深究。本文主要介绍，如何使用`docker-compose`来编排这些组件，这种方式实用于测试环境的部署或者你的个人`sideproject`的部署。

本文认为你已经了解`docker`和`docker-compose`的一些基本知识，若你不了解，可阅读下面这些资料：
...
在docker-compose中，容器的启动有两种方法，一种是直接使用公共的镜像来启动容器，另一种是通过我们自己变得的Dockerfile。因为我们需要安装需要的工具包和初始化相关配置，web和celery组件我们使用自定义的Dockerfile方式。
...
到此，使用docker来部署django技术栈服务就说完了，完整的项目代码，大家可参阅 [docker-django-demo](https://github.com/pylixm/docker-django-demo/tree/docker-compose-production3)。

文章开始已经说了，该部署方式不适合大型网站的线上生产服务，耦合度太高，不好维护等存在着许多问题。但是，若是部署自己的sideproject，在硬件资源有限的情况的下还是非常不错的。除了减少环境部署搭建的麻烦外，迁移起来也是很方便的。
...
- [](https://github.com/pylixm/docker-django-demo/tree/docker-compose-production)